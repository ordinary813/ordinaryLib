...
parser.add_argument(
    "save_models_dir",
    type=str,
    help="Path to the directory of the saved models."
)
...
def train_save_GPT2(model, tokenizer, tokenized_dataset, prompt='The movie was'):
    ...
    save_directory = os.path.join(save_models_dir, save_directory)
...
positive_dir = os.path.join(save_models_dir, "gpt2-positive-results")
positive_model = GPT2LMHeadModel.from_pretrained(positive_dir)
positive_tokenizer = GPT2Tokenizer.from_pretrained(positive_dir)

negative_dir = os.path.join(save_models_dir, "gpt2-negative-results")
negative_model = GPT2LMHeadModel.from_pretrained(negative_dir)
negative_tokenizer = GPT2Tokenizer.from_pretrained(negative_dir)
